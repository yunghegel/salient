...
public class TreeEx<N extends TreeEx.Node, V> extends WidgetGroup
{

    static private final Vector2 tmp = new Vector2();

    Tree.TreeStyle style;
    public Array<N> rootNodes = new Array();
    public Selection<N> selection;
    public float ySpacing = 4, iconSpacingLeft = 2, iconSpacingRight = 2, paddingLeft, paddingRight, indentSpacing;
    public float prefWidth, prefHeight;
    public boolean sizeInvalid = true;
    public N foundNode, overNode;
    N rangeStart;
    private ClickListener clickListener;

    public TreeEx(Skin skin) {
        this(skin.get( Tree.TreeStyle.class));
    }

    public TreeEx(Skin skin , String styleName) {
        this(skin.get(styleName ,  Tree.TreeStyle.class));
    }

    public TreeEx( Tree.TreeStyle style) {
        ...
        addListener(clickListener = new ClickListener()
        {
            public void clicked(InputEvent event , float x , float y) {
                ...
                if (selection.getMultiple() && selection.notEmpty() && UIUtils.shift()) {
                    ...
                    N rangeStart = TreeEx.this.rangeStart;
                    ...
                    TreeEx.this.rangeStart = rangeStart;
                    return;
                }
            ...
            public void exit(InputEvent event , float x , float y , int pointer , @Null Actor toActor) {
                ...
                if (toActor == null || !toActor.isDescendantOf(TreeEx.this)) setOverNode(null);
            }
        });
    }
    ...
    /**
     * Returns the drawable for the expand icon. The default implementation returns {@link   Tree.TreeStyle#plusOver} or
     * {@link   Tree.TreeStyle#minusOver} on the desktop if the node is the {@link #getOverNode() over node}, the mouse is left of
     * {@code iconX}, and clicking would expand the node.
     *
     ...
     *
     * @see TreeEx.Node#updateChildren()
     */
    ...
    static boolean findExpandedValues(Array<? extends TreeEx.Node> nodes , Array values) {
        boolean expanded = false;
        for (int i = 0, n = nodes.size; i < n; i++) {
            TreeEx.Node node = nodes.get(i);
    ...
    static @Null TreeEx.Node findNode(Array<? extends TreeEx.Node> nodes , Object value) {
        for (int i = 0, n = nodes.size; i < n; i++) {
            TreeEx.Node node = nodes.get(i);
        ...
        for (int i = 0, n = nodes.size; i < n; i++) {
            TreeEx.Node node = nodes.get(i);
            TreeEx.Node found = findNode(node.children , value);
    ...
    static void collapseAll(Array<? extends TreeEx.Node> nodes) {
        for (int i = 0, n = nodes.size; i < n; i++) {
            TreeEx.Node node = nodes.get(i);
    ...
    static void expandAll(Array<? extends TreeEx.Node> nodes) {
        for (int i = 0, n = nodes.size; i < n; i++) {
    ...
    /**
     * A {@link  TreeEx} node which has an actor and value.
     * <p>
     ...
     * @author Nathan Sweet
     */
    static abstract public class Node<N extends TreeEx.Node, V, A extends Actor>
    {
         ...
         */
        protected int addToTree(TreeEx<N, V> tree , int actorIndex) {
        ...
        protected void removeFromTree(TreeEx<N, V> tree , int actorIndex) {
        ...
        public void insert(int childIndex , N node) {
            ...
            if (tree != null) {
                ...
                else if (childIndex < children.size - 1) actorIndex = children.get(childIndex + 1).actor.getZIndex();
                else {
         ...
         * are expanded.
         */
        public @Null TreeEx<N, V> getTree() {
            Group parent = actor.getParent();
            if (parent instanceof TreeEx) return (TreeEx) parent;
            return null;
        }

        public void setActor(A newActor) {
            if (actor != null) {
                TreeEx<N, V> tree = getTree();
                if (tree != null) {
        ...
        public @Null N findNode(V value) {
            ...
            return (N) TreeEx.findNode(children , value);
        }

        /** Collapses all nodes under and including this node. */
        public void collapseAll() {
            setExpanded(false);
            TreeEx.collapseAll(children);
        }

        /** Expands all nodes under and including this node. */
        public void expandAll() {
            setExpanded(true);
            if (children.size > 0) TreeEx.expandAll(children);
        }

        /** Expands all parent nodes of this node. */
        public void expandTo() {
            TreeEx.Node node = parent;
            while (node != null) {
                node.setExpanded(true);
                node = node.parent;
            }
        ...
        public void findExpandedValues(Array<V> values) {
            if (expanded && !TreeEx.findExpandedValues(children , values)) values.add(value);
        }
